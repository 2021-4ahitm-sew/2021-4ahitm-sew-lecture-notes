...
docker run --restart unless-stopped --name postgres -e POSTGRES_PASSWORD=postgres -d -p 5432:5432 -v $HOME/databases/postgres:/var/lib/postgresql/data postgres
----
...
* JDBC drive (z.B. `quarkus-jdbc-postgresql`)
...
* Entities annotieren (`@Entity`, `@Id`,...)
* `EntityManager` in Service injecten (Achtung: das gehört _eigentlich_ ins Repository!)
* Service Methode mit `@Transactional` annotieren (ditto: Repository)